...
IF OBJECT_ID('TICKET_PROMEDIO_MENSUAL', 'V') IS NOT NULL
	DROP VIEW TICKET_PROMEDIO_MENSUAL;
GO
CREATE VIEW TICKET_PROMEDIO_MENSUAL
AS
SELECT 
...
IF OBJECT_ID('CANTIDAD_UNIDADES_PROMEDIO', 'V') IS NOT NULL
	DROP VIEW CANTIDAD_UNIDADES_PROMEDIO;
GO
CREATE VIEW CANTIDAD_UNIDADES_PROMEDIO
AS 
SELECT
...
IF OBJECT_ID('PORCENTAJE_ANUAL_VENTAS', 'V') IS NOT NULL
	DROP VIEW PORCENTAJE_ANUAL_VENTAS;
GO
CREATE VIEW PORCENTAJE_ANUAL_VENTAS 
AS
SELECT
	t.ANIO AS 'ANIO',
	t.CUATRIMESTRE AS 'CUATRIMESTRE',
...
IF OBJECT_ID('VENTAS_POR_TURNO', 'V') IS NOT NULL
	DROP VIEW VENTAS_POR_TURNO;
GO
CREATE VIEW VENTAS_POR_TURNO
AS
SELECT
	t.ANIO AS 'ANIO',
	t.MES AS 'MES',
...
IF OBJECT_ID('PORCENTAJE_DESCUENTO_APLICADO', 'V') IS NOT NULL
	DROP VIEW PORCENTAJE_DESCUENTO_APLICADO;
GO
CREATE VIEW PORCENTAJE_DESCUENTO_APLICADO
AS
SELECT
	t.ANIO AS 'ANIO',
	t.MES AS 'MES',
...
IF OBJECT_ID('TRES_CATEGORIAS_MAYOR_DESCUENTO', 'V') IS NOT NULL
	DROP VIEW TRES_CATEGORIAS_MAYOR_DESCUENTO;
GO
CREATE VIEW TRES_CATEGORIAS_MAYOR_DESCUENTO
AS
SELECT
	t.ANIO AS 'ANIO',
	t.CUATRIMESTRE AS 'CUATRIMESTRE',
...
IF OBJECT_ID('PORCENTAJE_CUMPLIMIENTO_ENVIOS', 'V') IS NOT NULL
	DROP VIEW PORCENTAJE_CUMPLIMIENTO_ENVIOS;
GO
...
-- IF OBJECT_ID('ENVIOS_POR_RANGO', 'V') IS NOT NULL
-- 	DROP VIEW ENVIOS_POR_RANGO;
-- GO
-- CREATE VIEW ENVIOS_POR_RANGO
-- AS
-- SELECT 
-- 	t.ANIO AS 'ANIO',
-- 	t.CUATRIMESTRE AS 'CUATRIMESTRE',
...
SELECT 
    t.ANIO AS 'ANIO',
    t.CUATRIMESTRE AS 'CUATRIMESTRE',
    CONCAT(re.RANGO_MENOR, '-', re.RANGO_MAYOR) AS 'RANGO ETAREO',
    SUM(he.CANT_ENVIOS) AS 'CANTIDAD DE ENVIOS'
FROM [QUICK_SORT].[BI_HECHO_ENVIO] he
JOIN [QUICK_SORT].[BI_TIEMPO] t ON he.TIEMPO_ID = t.TIEMPO_ID
JOIN [QUICK_SORT].[BI_RANGO_ETAREO] re ON he.RANGO_ETAREO_ID = re.RANGO_ETAREO_ID
GROUP BY t.ANIO, t.CUATRIMESTRE, re.RANGO_MENOR, re.RANGO_MAYOR
GO


-- 9 TOP 5 LOCALIDADES CON MAYOR COSTO DE ENVIO -- NO ME QUEDA CLARO POR LO QUE SE ESTA ORDENANDO

IF OBJECT_ID('VW_TOP5_LOCALIDADES_COSTO_ENVIO', 'V') IS NOT NULL
	DROP VIEW VW_TOP5_LOCALIDADES_COSTO_ENVIO;
GO
CREATE VIEW VW_TOP5_LOCALIDADES_COSTO_ENVIO AS
SELECT TOP 5
    l.LOCALIDAD,
    SUM(he.COSTO) AS TOTAL_COSTO_ENVIO
FROM 
    QUICK_SORT.BI_HECHO_ENVIO he
JOIN 
    QUICK_SORT.BI_LOCALIDAD l ON he.LOCALIDAD_ID = l.LOCALIDAD_ID
GROUP BY 
    l.LOCALIDAD
ORDER BY 
    SUM(he.COSTO) DESC;
...
-- está ordenando las localidades por el costo total de envío (SUM(he.COSTO)) en orden descendente (DESC). Las cinco localidades con el costo total de envío más alto se mostrarán en la vista.

--10 TOP 3 SUCURSALES CON MAYOR IMPORTE DE PAGO EN CUOTAS || esta mal, no me deja crearla por el having

-- IF OBJECT_ID('TOP3_SUCURSALES_MAYOR_PAGO_CUOTA', 'V') IS NOT NULL
-- 	DROP VIEW TOP3_SUCURSALES_MAYOR_PAGO_CUOTA;
-- GO
-- ALTER VIEW TOP3_SUCURSALES_MAYOR_PAGO_CUOTA AS
-- SELECT TOP 3
-- 	t.ANIO,
-- 	t.MES,
-- 	hp.MEDIO_PAGO_ID
-- FROM [QUICK_SORT].[BI_HECHO_PAGO] hp JOIN 
-- [QUICK_SORT].[BI_TIEMPO] t ON t.TIEMPO_ID = hp.TIEMPO_ID
-- WHERE  (SELECT COUNT(*)
...
CREATE VIEW TOP3_SUCURSALES_MAYOR_PAGO_CUOTA AS
SELECT TOP 3 WITH TIES
    t.ANIO,
    t.MES,
    hp.MEDIO_PAGO_ID,
    hp.SUCURSAL_ID,
    SUM(hp.TOTAL_CUOTAS) AS TOTAL_CUOTAS
FROM [QUICK_SORT].[BI_HECHO_PAGO] hp
JOIN [QUICK_SORT].[BI_TIEMPO] t ON t.TIEMPO_ID = hp.TIEMPO_ID
GROUP BY t.ANIO, t.MES, hp.MEDIO_PAGO_ID, hp.SUCURSAL_ID
ORDER BY SUM(hp.TOTAL_CUOTAS) DESC;
GO


--11 promedio importe cuota por rango etareo cliente
IF OBJECT_ID('PROMEDIO_CUOTA_POR_RANGO', 'V') IS NOT NULL
	DROP VIEW PROMEDIO_CUOTA_POR_RANGO;
GO
CREATE VIEW PROMEDIO_CUOTA_POR_RANGO AS 
SELECT
...
--11 segunda versión con rango etáreo
...
SELECT
    ...
    SUM(hp.TOTAL_CUOTAS) / SUM(hp.CANTIDAD_CUOTAS) AS 'PROMEDIO CUOTA'
FROM [QUICK_SORT].[BI_HECHO_PAGO] hp
JOIN [QUICK_SORT].[BI_RANGO_ETAREO] re ON hp.RANGO_ETAREO_ID = re.RANGO_ETAREO_ID
GROUP BY re.RANGO_MENOR, re.RANGO_MAYOR
GO



--12 PORCENTAJE DESCUENTO APLICADO POR MEDIO PAGO
-- IF OBJECT_ID('DESCUENTO_POR_MEDIO_PAGO', 'V') IS NOT NULL
-- 	DROP VIEW DESCUENTO_POR_MEDIO_PAGO;
-- GO
-- CREATE VIEW DESCUENTO_POR_MEDIO_PAGO AS
-- SELECT
-- 	t.ANIO AS 'ANIO',
-- 	t.CUATRIMESTRE AS 'CUATRIMESTRE',
...
-- SELECT * FROM PROMEDIO_CUOTA_POR_RANGO
...
--DROPEA VISTA
-- IF OBJECT_ID('QUICK_SORT.VW_3_PORCENTAJE_ANUAL_VENTAS_RANGO_ETARIO', 'V') IS NOT NULL
-- 	DROP VIEW QUICK_SORT.VW_3_PORCENTAJE_ANUAL_VENTAS_RANGO_ETARIO;

-- IF OBJECT_ID('QUICK_SORT.VW_CANTIDAD_ENVIOS_RANGO_ETARIO', 'V') IS NOT NULL
-- 	DROP VIEW QUICK_SORT.VW_CANTIDAD_ENVIOS_RANGO_ETARIO;

-- IF OBJECT_ID('QUICK_SORT.VW_TICKET_PROMEDIO_MENSUAL', 'V') IS NOT NULL
-- 	DROP VIEW QUICK_SORT.VW_TICKET_PROMEDIO_MENSUAL;

-- IF OBJECT_ID('QUICK_SORT.vw_TicketPromedioMensual', 'V') IS NOT NULL
-- 	DROP VIEW QUICK_SORT.vw_TicketPromedioMensual;

-- IF OBJECT_ID('VW_TOP5_LOCALIDADES_COSTO_ENVIO', 'V') IS NOT NULL
-- 	DROP VIEW QUICK_SORT.VW_TOP5_LOCALIDADES_COSTO_ENVIO;
