...
BEGIN
	...
	INSERT INTO [QUICK_SORT].[ESTADO_POR_ENVIO](ESTADO_ID, ENVIO_ID)
		...
		JOIN [QUICK_SORT].[ENVIO] en ON en.ENVIO_HORA_INICIO = m.ENVIO_HORA_INICIO AND en.ENVIO_FECHA_ENTREGA = m.ENVIO_FECHA_PROGRAMADA
	...
	INSERT INTO [QUICK_SORT].[DETALLE_TICKET](DETALLE_TICKET_TICKET_ID, PRODUCTO_ID, DETALLE_CANTIDAD,
		...
		t1.TICKET_SUBTOTAL_PRODUCTOS = m.TICKET_SUBTOTAL_PRODUCTOS
		join [QUICK_SORT].[EMPLEADO] e on t1.TICKET_EMPLEADO_LEGAJO = e.EMPLEADO_LEGAJO
		join [QUICK_SORT].[PRODUCTO] p ON p.PRODUCTO_NOMBRE = m.PRODUCTO_NOMBRE AND p.PRODUCTO_MARCA = m.PRODUCTO_MARCA AND
		p.PRODUCTO_PRECIO = m.PRODUCTO_PRECIO
		WHERE m.EMPLEADO_DNI = e.EMPLEADO_DNI AND m.PRODUCTO_NOMBRE IS NOT NULL AND m.PRODUCTO_MARCA IS NOT NULL
    ...
    INSERT INTO [QUICK_SORT].[DETALLE_PAGO] (DET_PAGO_CLIENTE_ID,DET_PAGO_NRO_TARJETA,DET_PAGO_FECHA_VEN_TARJETA,DET_PAGO_CUOTAS)
        SELECT DISTINCT c.CLIENTE_ID,m.PAGO_TARJETA_NRO,m.PAGO_TARJETA_FECHA_VENC,m.PAGO_TARJETA_CUOTAS
        FROM [gd_esquema].[Maestra] m 
        JOIN [QUICK_SORT].[CLIENTE] c ON c.CLIENTE_DNI = m.CLIENTE_DNI AND c.CLIENTE_APELLIDO = m.CLIENTE_APELLIDO 
