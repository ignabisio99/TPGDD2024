...
IF NOT EXISTS(SELECT name FROM sys.procedures WHERE name='CREATE_INDEXES')
	EXEC('CREATE PROCEDURE [QUICK_SORT].[CREATE_INDEXES] AS BEGIN SET NOCOUNT ON; END');
GO

ALTER PROCEDURE [QUICK_SORT].[CREATE_INDEXES]
AS
BEGIN
	CREATE INDEX INDEX_CLIENTE_DNI ON [QUICK_SORT].[CLIENTE](CLIENTE_DNI);
	CREATE INDEX INDEX_PRODUCTO ON [QUICK_SORT].[PRODUCTO](PRODUCTO_NOMBRE);
	CREATE INDEX INDEX_DIRECCION ON [QUICK_SORT].[DIRECCION](DIRECCION)
END
GO

EXEC [QUICK_SORT].[CREATE_INDEXES]

IF NOT EXISTS(SELECT name FROM sys.procedures WHERE name='MIGRAR')
	EXEC('CREATE PROCEDURE [QUICK_SORT].[MIGRAR] AS BEGIN SET NOCOUNT ON; END');
GO
...
BEGIN
	...
	INSERT INTO [QUICK_SORT].[DIRECCION] (LOCALIDAD_ID, DIRECCION)
		...
		JOIN [QUICK_SORT].[LOCALIDAD] l ON SUPER_LOCALIDAD = l.LOCALIDAD
		JOIN [QUICK_SORT].[PROVINCIA] p ON p.PROVINCIA_ID = l.PROVINCIA_ID
		WHERE SUPER_DOMICILIO IS NOT NULL AND m.SUPER_PROVINCIA = p.PROVINCIA
		UNION
		SELECT DISTINCT LOCALIDAD_ID, SUCURSAL_DIRECCION
		FROM [gd_esquema].[Maestra] m
		JOIN [QUICK_SORT].[LOCALIDAD] l ON SUCURSAL_LOCALIDAD = l.LOCALIDAD
		JOIN [QUICK_SORT].[PROVINCIA] p ON p.PROVINCIA_ID = l.PROVINCIA_ID
		WHERE SUCURSAL_DIRECCION IS NOT NULL AND m.SUCURSAL_PROVINCIA = p.PROVINCIA
		UNION
		SELECT DISTINCT LOCALIDAD_ID, CLIENTE_DOMICILIO
		FROM [gd_esquema].[Maestra] m
		JOIN [QUICK_SORT].[LOCALIDAD] l ON CLIENTE_LOCALIDAD = l.LOCALIDAD
		JOIN [QUICK_SORT].[PROVINCIA] p ON p.PROVINCIA_ID = l.PROVINCIA_ID
		WHERE CLIENTE_DOMICILIO IS NOT NULL AND m.CLIENTE_PROVINCIA = p.PROVINCIA
	...
	INSERT INTO [QUICK_SORT].[CLIENTE] (CLIENTE_DIRECCION_ID,CLIENTE_DNI,CLIENTE_NOMBRE,CLIENTE_APELLIDO,
		...
		SELECT DISTINCT d.DIRECCION_ID,m.CLIENTE_DNI,m.CLIENTE_NOMBRE,m.CLIENTE_APELLIDO,
		m.CLIENTE_FECHA_REGISTRO,m.CLIENTE_TELEFONO,m.CLIENTE_MAIL,m.CLIENTE_FECHA_NACIMIENTO
		FROM [gd_esquema].[Maestra] m
		JOIN [QUICK_SORT].[PROVINCIA] p ON m.CLIENTE_PROVINCIA = p.PROVINCIA
		JOIN [QUICK_SORT].[LOCALIDAD] l ON m.CLIENTE_LOCALIDAD = l.LOCALIDAD and l.PROVINCIA_ID = p.PROVINCIA_ID
        JOIN [QUICK_SORT].[DIRECCION] d ON m.CLIENTE_DOMICILIO = d.DIRECCION and d.LOCALIDAD_ID = l.LOCALIDAD_ID
    ...
    INSERT INTO [QUICK_SORT].[PAGO] (PAGO_TICKET_ID, PAGO_MEDIO_PAGO_ID, DETALLE_PAGO_ID, PAGO_FECHA, PAGO_IMPORTE,
			PAGO_DESCUENTO_APLICADO)
       SELECT DISTINCT t.TICKET_ID, mp.PAGO_MEDIO_PAGO_ID, d.DETALLE_PAGO_ID, m.PAGO_FECHA, m.PAGO_IMPORTE, m.PAGO_DESCUENTO_APLICADO 
	   FROM [gd_esquema].[Maestra] m
	   JOIN [QUICK_SORT].[TICKET] t ON t.TICKET_NRO = m.TICKET_NUMERO AND t.TICKET_FECHA_HORA = m.TICKET_FECHA_HORA AND
				t.TICKET_TOTAL = m.TICKET_TOTAL_TICKET
	   JOIN [QUICK_SORT].[PAGO_MEDIO_PAGO] mp ON mp.PAGO_MEDIO_PAGO = m.PAGO_MEDIO_PAGO
	   LEFT JOIN [QUICK_SORT].[DETALLE_PAGO] d ON d.DET_PAGO_NRO_TARJETA = m.PAGO_TARJETA_NRO 
	   AND d.DET_PAGO_CUOTAS = m.PAGO_TARJETA_CUOTAS AND d.DET_PAGO_FECHA_VEN_TARJETA = m.PAGO_TARJETA_FECHA_VENC
	   WHERE m.PAGO_FECHA IS NOT NULL AND m.PAGO_IMPORTE IS NOT NULL AND m.PAGO_MEDIO_PAGO IS NOT NULL
END
GO
...
SELECT * FROM [QUICK_SORT].[TICKET] WHERE TICKET_ID = 121
SELECT * FROM [gd_esquema].[Maestra] WHERE TICKET_NUMERO = 1353281398

SELECT * FROM [QUICK_SORT].[PAGO] WHERE PAGO_TICKET_ID = 121

SELECT * FROM [QUICK_SORT].[DETALLE_PAGO] WHERE DETALLE_PAGO_ID = 2123 OR DETALLE_PAGO_ID = 4397

SELECT * FROM [QUICK_SORT].[CLIENTE] WHERE CLIENTE_ID = 2481 OR CLIENTE_ID = 5143

SELECT * FROM [gd_esquema].[Maestra] WHERE CLIENTE_DNI = 73612750 AND CLIENTE_APELLIDO = 'Rodríguez'

SELECT * FROM [QUICK_SORT].[DIRECCION] WHERE DIRECCION_ID = 2464 OR DIRECCION_ID = 5115
SELECT * FROM [QUICK_SORT].[LOCALIDAD] WHERE LOCALIDAD_ID = 2231 OR LOCALIDAD_ID = 4715